...
c.add_wasm("add_one",        # name of the function
           ...
           [Bit(0), Bit(1),
            Bit(2), Bit(3), Bit(4)]) # list of bits where the wasm op will be added to

# When adding functions with multible parameters this can be done in the same way:

c.add_wasm("multi",          # name of the function
           wfh,              # wasm file handler
...
c.add_wasm_to_reg("multi",   # function name
                  wfh,       # wasm file handler
                  [c0, c1],  # register for each input parameter
                  [c2])      # register for the result
c.add_wasm_to_reg("add_one", # function name
                  wfh,       # wasm file handler 
...
# if you want to send the to a backend, you can do this with:    
