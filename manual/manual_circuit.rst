...
.. warning:: The unitary matrix of a quantum circuit is of dimension :math:`(2^n \times 2^n)` where $n$ is the number of qubits. The statevector will be a column vector with length :math:`2^n` . Due to this exponential scaling it will in general be very inefficent to compute the unitary (or statevector) of a circuit. These functions are intended to be used for sanity checks and spotting mistakes in small circuits.
