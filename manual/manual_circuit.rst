...
The primary use for tableau in ``pytket`` is as a scalable means of specifying a Clifford unitary for insertion into a circuit as a Box. This can then be decomposed into basic gates during compilation.
...
.. note:: The current decomposition method for tableaux is particularly inefficient in terms of gate count, so consider using higher optimisation levels when compiling to help reduce the gate cost.

The data structure used here for tableau is intended for compilation use. For fast simulation of Clifford circuits, we recommend using the :py:class:`StimBackend` from ``pytket-stim`` or :py:class:`AerBackend` from ``pytket-qiskit``. Future versions of ``pytket`` may include improved decompositions from tableaux, as well as more flexible tableaux to represent stabilizer states, isometries, and diagonalisation circuits.
